...
print("=" * 60)
print(f"ANALISE ARM x IAG - WDI REAL ({len(df)} paises)")
print("=" * 60)
...
pares = [
    ("agricultura_pib_pct", "internet_pop_pct", "Agricultura vs Internet"),
    ("gini", "internet_pop_pct", "Desigualdade vs Internet"),
    ("educ_superior_pct", "gini", "Educacao vs Desigualdade"),
...
if ci_list:
    ...
    plt.suptitle("Correlacoes Nao-Obvias - Dados WDI", fontsize=13, fontweight="bold")
...
if len(X) > 20:
    ...
    if rf:
        imp = pd.DataFrame({"variavel": X.columns, "importancia": rf.feature_importances_}).sort_values("importancia", ascending=False)
        imp.to_csv(OUT / "tabela_feature_importance_rf.csv", index=False)
        resultados["top_features"] = imp.head(15).to_dict("records")

        plt.figure(figsize=(9, 7))
        imp.head(15).plot(x="variavel", y="importancia", kind="barh", color="steelblue", legend=False)
...
with open(OUT / "resultados_analise.json", "w") as f:
...
5. VALIDACAO: R2={resultados.get("rkf_mean","N/A")}+-{resultados.get("rkf_std","N/A")}
