# -*- coding: utf-8 -*-
"""
AUTO SWARM BUILDER v1.0 - Sem necessidade de DI (construtor de swarms).

Analise:
- AutoSwarmBuilder cria configuracao de multi-agentes (swarms)
- Nao importa core.state_manager, core.event_bus ou qualquer infra
- Dependencias: json, logging, typing, dataclasses
- E um builder/configurador, nao um servico de infraestrutura
...
class AgentSpec:
    ...
    model_name: str = "big-pickle"; max_loops: int = 1; tools: list = field(default_factory=list)

@dataclass
class SwarmRouterConfig:
    name: str; description: str; agents: list; swarm_type: str
    rules: Optional[str] = None; task: str = ""
...
class AutoSwarmBuilder:
    def __init__(self, name="auto-swarm-builder", description="Auto Swarm Builder",
        ...
        self.name=name; self.description=description; self.verbose=verbose
        self.max_loops=max_loops; self.model_name=model_name
        self.execution_type=execution_type; self.system_prompt=system_prompt
        self.agents_pool=[]
    def run(self, task: str, boss_agent_fn) -> Any:
        if self.verbose: logger.info(f"Analyzing task: {task[:100]}...")
        try: config = json.loads(boss_agent_fn(self.system_prompt,task))
        except json.JSONDecodeError as e: raise ValueError(f"Invalid JSON: {e}")
        agents = [AgentSpec(name=ad["name"],description=ad["description"],
            system_prompt=ad["system_prompt"]) for ad in config.get("agents",[])]
        if self.execution_type=="return-agents": return agents
        elif self.execution_type=="return-config":
            return SwarmRouterConfig(name=config.get("name",self.name),description=config.get("description",self.description),
                agents=agents,swarm_type=config.get("swarm_type","sequential"),rules=config.get("rules"),task=task)
        elif self.execution_type=="return-json": return boss_agent_fn(self.system_prompt,task)
        else: raise ValueError(f"Unknown execution_type: {self.execution_type}")
    def batch_run(self, tasks: list, boss_agent_fn) -> list:
        return [self.run(t,boss_agent_fn) for t in tasks]

class BuiltInTaskAnalyzers:
    @staticmethod
    def research_task_analyzer() -> dict:
        return {"name":"research-swarm","swarm_type":"research_analysis_synthesis","agents":[
    ...
    def code_review_task_analyzer() -> dict:
        return {"name":"code-review-swarm","swarm_type":"council_of_judges","agents":[
    ...
    def content_creation_task_analyzer() -> dict:
        return {"name":"content-swarm","swarm_type":"sequential","agents":[
