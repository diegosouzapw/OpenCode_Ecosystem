# -*- coding: utf-8 -*-
"""
EVOLUTION LOOP v5.0-DI - Refatorado com Injeção de Dependência.

Mudancas:
- FeedbackLoopEngine aceita state_manager via construtor
- SocialDiagnosisEngine aceita state_manager via construtor
- EvolutionLoopRunner aceita state_manager via construtor
...
class FeedbackLoopEngine:
    """Core feedback loop with DI support."""
    ...
    def _load_state(self):
        outcomes_data = self._sm.get("outcomes", default={})
        if outcomes_data:
            try:
                self.outcomes = [OutcomeRecord(**o) for o in outcomes_data.get("outcomes", [])]
            except (TypeError, KeyError):
                pass
        learnings_data = self._sm.get("learnings", default={})
        if learnings_data:
            try:
    ...
    def get_skill_generation_candidates(self) -> list[dict]:
        ...
        state = self._sm.get("ecosystem-state", default={})
        if state:
            ...
            if low_score:
                candidates.append({
                    ...
                    "rationale": "Components with low scores need improvement skills",
                    "confidence": 0.8
                })
        return candidates
...
class SocialDiagnosisEngine:
    """Multi-agent diagnosis engine with DI support."""

    def __init__(self, feedback_loop: FeedbackLoopEngine, state_manager: Optional[IStateManager] = None):
        self._sm = state_manager or Container.instance().resolve('state_manager')
    ...
    def run_diagnosis(self, task: str = "Full ecosystem health check") -> dict:
        try:
            from social_algorithms import Agent, SocialAlgorithms
        except ImportError:
            # Fallback simple se social_algorithms não for importável
            class Agent:
            ...
            class SocialResult:
                def __init__(self, final_outputs, success=True, execution_time=0.1, comm_log=None):
                    self.final_outputs = final_outputs
                    self.success = success
                    self.execution_time = execution_time
                    self.communication_log = comm_log or []

            class SocialAlgorithms:
                def __init__(self, name, agents, social_algorithm):
                    self.name = name
                    self.agents = agents
                    self.social_algorithm = social_algorithm
                def run(self, task):
                    outputs = self.social_algorithm(self.agents, task)
                    return SocialResult(outputs)
        ...
        diagnosis = {
            ...
            "communication_log": getattr(result, "communication_log", [])
        }
        ...
        self.fb.record_outcome(
            ...
            duration_ms=getattr(result, "execution_time", 0.1) * 1000,
...
class EvolutionLoopRunner:
    """Runs the complete evolution cycle with DI support."""

    def __init__(self, state_manager: Optional[IStateManager] = None):
        self._sm = state_manager or Container.instance().resolve('state_manager')
        self.fb = FeedbackLoopEngine(state_manager=self._sm)
        self.diagnosis = SocialDiagnosisEngine(self.fb, state_manager=self._sm)
    ...
    def _run_health_check(self) -> float:
        try:
            ...
            orch = SyncOrchestrator(state_manager=self._sm)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Evolution Loop v5.0-DI - Feedback Loop Real")
