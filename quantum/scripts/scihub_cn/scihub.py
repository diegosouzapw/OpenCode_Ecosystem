...
def construct_download_setting():
 ...
 -p(--proxy),-o(--output),-e(--engine)，-l(--limit)，, 
 google_scholar, baidu_xueshu, publons, science_direct
 ''')
...
class SciHub(object):
 ...
 """
 if 'response' in info: # info, {"response": "https://.../downloads/..."}
 res = info['response']
 ...
 # scihubdoi 10.1609/aimag.v18i4.1324
 else:
 ...
 logger.info('，pdf : %s (resolved url %s).'
 % (info['title'], url))
 return {
 'err': '，pdf :%s (resolved url %s).'
 % (info['title'], url)
 }, None
 ...
 raise Exception('http://tool.yovisun.com/scihub/，！')
 
 logger.info(f" {self.base_url + identifier} ...")
 s = self._get_soup(res.content)
 ...
 logger.info(identifier + f' scihub：{e}')
 return None
 ...
 'class'][0] == 'gs_ct1' and tag.text == '[]', recursive=True)
 if not res_set:
 raise VerificationError("Error:google scholar ")
 for tag in res_set:
 ...
 return results
 i += 10
 if len(res_set) + len(citations) < 10: # 10
 break
...
def main():
 ...
 if 'bibtex' in attr:
 logger.info("info:%s..." % value[value.rfind('\\') + 1:])
